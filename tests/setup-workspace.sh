...
LOCAL_MIRROR="${WORKSPACE_DIR}/mirror"
...
project_clean() {
  ...
  git -C "${PROJECT_CLEAN}" remote add remote2 "${UPSTREAM}"
...
project_new_local_commit() {
  ...
  git -C "${PROJECT_NEW_LOCAL_COMMIT}" remote add remote2 "${UPSTREAM}"
...
project_new_remote_commit() {
  ...
  git -C "${PROJECT_NEW_REMOTE_COMMIT}" remote add remote2 "${UPSTREAM}"
...
project_new_files() {
  ...
  git -C "${PROJECT_NEW_FILES}" remote add remote2 "${UPSTREAM}"
...
project_changed_files() {
  ...
  git -C "${PROJECT_CHANGED_FILES}" remote add remote2 "${UPSTREAM}"
